...
if [ "$MVN_ARG_LINE" != "" ] ; then
    ...
    "$mvnBin" -v
    echo
    projects=( "*-model" "*-kjar" "first-task-service")
...
if [[ "$@" =~ "docker" ]]; then
    ...
    docker run -d -p 8090:8090 --name first-task-service apps/first-task-service:1.0-SNAPSHOT
elif [[ "$@" =~ "openshift" ]]; then
    echo "Launching the application on OpenShift..."
    
    oc new-app first-task-service:1.0-SNAPSHOT
    oc expose svc/first-task-service
else

	echo "Launching the application locally..."
	pattern="first-task-service"
	files=( $pattern )
	cd ${files[0]}
